    ...
    <div class="addUsers">
        ...
        <div class="form">
            ...
            >
                ...
                >
                    <a-textarea v-model:value="formState.USER_INTRO" />
                </a-form-item>
                <a-form-item
                :label="t('message.password')"
                :rules="[{ required: true, message: t('message.enterPassword') }]"
                >
                    <a-input-password v-model:value="formState.USER_PASS" />
                </a-form-item>

                <a-form-item :wrapper-col="{ offset: 8, span: 16 }">
                <a-button v-if="isAdd" type="primary" html-type="submit">{{ $t('message.registerNewBuyer') }}</a-button>
...
export default {
  setup() {
    ...
    var info = ref(false);
    var isAdd = ref(true);
    isAdd.value = store.state.isAddUser;
    var formState = reactive(store.state.userInfo);
    function refresh(){
        location.reload();
    }
    // 提交成功
    const onFinish = (values) => {
      ...
      if(isAdd.value == true){
        ...
        api.postAPI('/admin/updateUser', formState)
            .then(
            (res) =>{
                if(res.status == 200){
                    info.value = true
