...
export async function outputPDF({
  ...
  async function addHeader(header, pdf, contentWidth) {
    if(headerData) {
      ...
      let canvasData = await toCanvas(newFooter, contentWidth);
      headerHeight = canvasData.height;
      headerData = canvasData.data;
    }
    pdf.addImage(headerData, "JPEG", 0, 0, contentWidth, headerHeight);
  }

  // 添加页脚（页脚类为pdf-footer-page）
  ...
  function traversingNodes(nodes) {
    for (let i = 0; i < nodes.length; ++i) {
      const one = nodes[i];
      // 需要判断跨页且内部存在跨页的元素（内部存在跨页元素类为divide-inside）
      const isDivideInside = one.classList && one.classList.contains("divide-inside");
      // 需要判断跨页且主动跨页的元素（主动跨页元素类为page-skip）
      const isPageSkip = one.classList && one.classList.contains("page-skip");
      // 图片元素不需要继续深入，作为深度终点
      const isIMG = one.tagName === "IMG";

      // 自定义深度终点（比如几行表格不能分开跨页）
      // 判断是否是设备监测详情的单元格，是的话进行深度终点判断
      const isDeviceInfos = one.classList && ((one.classList.contains("deviceInfos-table")));
      // 判断是否是设备信息汇总的table，是的话进行深度终点判断
      const isDetectionTr = one.classList && ((one.classList.contains("detection-tr")));
      // 判断是否是历史报警信息table的div，是的话进行深度终点判断
      const isHistoricalTableDivr = one.classList && ((one.classList.contains("historical-table-div")));
      // 判断是否是echarts图像的div，是的话进行深度终点判断
      ...
      // 对于深度终点元素进行处理
      else if (isIMG || isDeviceInfos || isTableCol || isDetectionTr || isHistoricalTableDivr || isDescriptionsEcharts) {
        // dom高度转换成生成pdf的实际高度
  ...
  // 添加页码
  function addPageNum(pdf, pageNum, now) {
    pdf.setFontSize(12);
    let width = 520;
    if(now >= 100) {
      width = width - 11;
    ...
    pdf.setFont("宋体");
    pdf.text("Page: " + now + "/" + pageNum, width, 52);
  }

  // 最后一页的遍历加页码
  if (isFinally){
    allPages = allPages.filter((val, index) => {
      return val !== 0;
    })
    ...
    for (let i = 0; i < allPages.length + 1; i++) {
      pdf.setPage(i + 1);
      // 添加页码
      await addPageNum(pdf, allPages.length + 1, i + 1);
    }
